  ...
  <div class="workingClass">
    ...
    <div class="workingContent">
      <div class="workingBody">
        <div id="item.id" class="workingTitle">项目类型</div>
    ...
    <!--
        新增项目类型
    -->
    <el-dialog :close-on-press-escape="false" title="新增项目类型" :visible.sync="addWorkingVisible"
    ...
    <!--
        修改工类
    -->
    <el-dialog :close-on-press-escape="false" title="修改项目类型" :visible.sync="editWorkingVisible"
...
<script>
  import {getProjcetList, addProject, editProject, deleteProject, checkProjectName} from '../../api/api';
  ...
  export default {
    data() {
      ...
      var validWorkerName = (rule, value, callback) => {
        if (value === '') {
          callback(new Error('请输入项目类型名称！'));
        } else if (!/^[^\s]+$/.test(value)) {
          callback(new Error('项目类型名称不能带有空格！'));
        } else {
          var parmas = {
            name: value
          }
          checkProjectName(parmas).then((res) => {
            if (res.data.code == 0) {
              callback()
            } else {
      ...
      return {
        ...
        addFormRules: {
          name: [
            ...
            {min: 1, max: 10, message: '项目类型名称长度不能超过10个字符', trigger: 'blur'}
          ]
        },
    ...
    methods: {
      ...
      // 删除
      deleteClick(id) {
        this.$confirm('确认删除该项目类型吗?', '提示', {
          type: 'warning'
        }).then(() => {
          let parmas = {'id': id};
          deleteProject(parmas).then((res) => {
            if (res.data.code == '0') {
              this.getList();
              myCallback.call(this,res);
              // this.$notify.success({
